...
def test_scope_outbox_includes_canonical():
    ...
    un directorio temporal (no depende de la rama marketing-outbox, que es float y
    no está en origin/main). El canónico lleva un claim afirmativo de red line #110
    que SÓLO vive ahí: si --scope outbox lo omitiera, el lint pasaría (falso verde)
    y la co-firma técnica evadiría el gate. El test afirma que el claim BLOQUEA.

    Usa tempfile en vez de tmp_path (fixture de pytest) para ser compatible con el
    runner sin dependencias de LucidFence (tests/run_tests.py). Toda la lógica vive
    DENTRO del with (el directorio temporal se borra al salir).
    """
    ...
    with _tf.TemporaryDirectory() as d:
        ...
        cto_input = Path(d) / ".cto_input_188.md"
...
def test_no_canonical_excludes_canonical():
