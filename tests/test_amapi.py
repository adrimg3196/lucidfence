...
def test_management_mode_and_ownership_propagation_and_fallback():
    """Verify management_mode and ownership map correctly and support declarative vs imperative fallback."""
    from lucidfence.core.location_source import LocationReport, LiveLocationSource
    from types import SimpleNamespace

    # 1. Propagation from LocationReport to DeviceState (engine behaviour)
    # Case A: Fields populated (declarative path)
    rep_decl = LocationReport(
        device_id="dev-decl",
        name="Android Declarative",
        platform="android",
        lat=40.42,
        lng=-3.71,
        status="active",
    ...
    state = DeviceState(
        device_id=rep_decl.device_id,
        name=rep_decl.name,
        platform=rep_decl.platform,
        management_mode=getattr(rep_decl, "management_mode", None),
        ownership=getattr(rep_decl, "ownership", None)
    )
    assert state.management_mode == DEVICE_OWNER
    assert state.ownership == COMPANY_OWNED
    assert supports_amapi(state) is True

    # Case B: Fields missing (imperative fallback path)
    rep_imp = LocationReport(
        device_id="dev-imp",
        name="Android Imperative",
        platform="android",
        lat=40.42,
        lng=-3.71,
        status="active",
    ...
    state_imp = DeviceState(
        device_id=rep_imp.device_id,
        name=rep_imp.name,
        platform=rep_imp.platform,
        management_mode=getattr(rep_imp, "management_mode", None),
        ownership=getattr(rep_imp, "ownership", None)
    )
    assert state_imp.management_mode is None
    assert state_imp.ownership is None
    assert supports_amapi(state_imp) is False

    # 2. LiveLocationSource parser (honesty & future-proofing)
    source = LiveLocationSource(org_id="org")

    # Case A: Future UEM payload with managementMode and ownership
    dev_with = {
        "id": "dev-future",
        "type": "android",
        "state": "ACTIVE",
        "displayName": "Future Phone",
        "summary": {
            "os": "android",
    ...
    report_with = source._to_report(dev_with)
    assert report_with.management_mode == DEVICE_OWNER
    assert report_with.ownership == COMPANY_OWNED

    # Case B: Current reality UEM payload without managementMode and ownership
    dev_without = {
        "id": "dev-current",
        "type": "android",
        "state": "ACTIVE",
    ...
    report_without = source._to_report(dev_without)
    assert report_without.management_mode is None
    assert report_without.ownership is None
