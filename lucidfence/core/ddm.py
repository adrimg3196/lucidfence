...
def _mock_decl_status(decl: dict) -> dict:
    ...
    Jamf no publica endpoint para subir declarations propias (`apply_ddm` las
    genera offline y el MDM las entrega). En mock el dispositivo no rechaza
    nada, así que todas las declarations del juego convergen en `accepted` —
    la misma forma que #89 leerá para decidir si el camino declarativo es
    viable (todas `accepted` => declarativo).
    """
    return {
...
def build_mock_apply_result(
    ...
    *,
    ...
    Devuelve las declarations estructuradas (como el modo offline real) más el
    estado por declaración que Jamf expondría tras la entrega, para que el
    engine pueda leer un resultado declarativo coherente sin un Jamf real.
    """
...
def build_mock_status_result(*, status_items=None) -> dict:
    ...
    Misma FORMA que la respuesta real de
    `GET /api/v1/ddm/{mid}/status-items`: `status_items` (nº) y `device_state`
    (mapeado desde los items suscritos). Los valores son ejemplo estables para
    que el test afirme la FORMA del contrato sin un tenant Jamf.
    """
    items = tuple(status_items) if status_items is not None else DEFAULT_STATUS_ITEMS
    device_state: dict = {}
    for name in items:
...
def build_mock_sync_result(*, reason: str = "declarations_updated") -> dict:
    ...
    Jamf responde 204 sin cuerpo; el mock devuelve el mismo contrato
    (`jamf_status=204`, `synced=True`) para que el engine lea un resultado
    declarativo sin Jamf real.
    """
