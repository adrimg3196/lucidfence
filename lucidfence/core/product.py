...
def _risk_from_engine(eng: Any, devices: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for d in devices:
        ...
        try:
            ...
            fired = []
        # score como número real para orden: None (unknown) va al final, nunca
        # se confunde con un 0 legítimo en las agregaciones de consumidores.
        raw_score = r.get("risk_score", None)
        sort_score = float(raw_score) if isinstance(raw_score, (int, float)) else -1.0
        rows.append({
