...
3. **Regla de Oro #1 (anti-check-fantasma, tarea t_26b7fac6):** antes de añadir
   cualquier contexto a `required_status_checks`, el workflow que lo emite debe
   estar MERGEADO en `origin/main` **y** tener un run verde en `main`. Validar con
   el pre-flight fail-closed:
   ```bash
   python3 scripts/ruleset_check_guard.py --diff <changeset.diff>   # exit 0 = APTO
   python3 scripts/ruleset_check_guard.py --audit-live              # audita el ruleset vivo (21249696)
   ```
   Un check "verde en mi rama/PR" NO cuenta: el deadlock de 2026-08-24 vino de
   exigir `autonomy-evidence`, cuyo workflow solo vivía en el PR #264 (rojo). El
   guard compara siempre contra `origin/main`, nunca contra la rama del PR
   (anti-falsos-positivos por staleness, t_925438a3). En review se exige el
   resultado del pre-flight en el cuerpo del PR.
